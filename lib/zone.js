...
/*
 * @param {object} opts - Options object
 * @param {string} opts.uri - The project URI
 *   i.e. https://www.googleapis.com/dns/v1/projects/core-falcon-443124
 * @param {string} opts.name - The zone i.e. example.com
 * @param {Auth} opts.auth - An authenticated token for making the request
 * @param {function} callback - An error first callback invoked when the
 *   operation has either failed or completed. If succesfull the call will
 *   return an array of records.
 */
module.exports.list = function list(opts, cb) {
  return request({
    uri: `${opts.uri}/managedZones/${opts.name}/changes`,
    method: 'GET',
    json: true,
    auth: {
      bearer: opts.auth,
    ...
    if (e) { return cb(e); }
    const optEntries = opts.entries || [];
    const entries = optEntries.concat(
      // eslint-disable-next-line comma-dangle
      body.changes.reduce((p, v) => p.concat(v.additions), [])
    );
